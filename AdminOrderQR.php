...
switch ($userRole) {
    ...
    case 'registeredUser':
        // Registered User
        $AdminQuery = $conn->query("SELECT AdminID FROM registered_user WHERE userName = '$username'");
        $AdminData = $AdminQuery->fetch_assoc();
...
$orderQR = generateQRCodeURL($gUserID, $AdminID, $foodVendorID);
$insertOrderQuery = $conn->prepare("INSERT INTO `order` (orderDate, orderStatus, totalAmount, orderQR, gUserID, AdminID, foodVendorID) VALUES (?, ?, ?, ?, ?, ?, ?)");
$insertOrderQuery->bind_param("ssdsiii", $orderDate, $orderStatus, $totalAmount, $orderQR, $gUserID, $AdminID, $foodVendorID);
...
function generateQRCodeURL($gUserID, $AdminID, $foodVendorID) {
...
<div class="box"><br><br>
    <h1 class="title">
        ...
        <input id="text" type="text" value="<?php echo generateQRCodeURL($gUserID, $AdminID, $foodVendorID); ?>" style="width:80%"></input>
        <br>
        <div id="qrcode"></div>
        <br>
        <a href="AdminOrderStatus.php?orderID=<?php echo $orderID; ?>" class="btnViewReceipt">View Order</a>
